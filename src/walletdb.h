...
public:
    ...
    bool WriteZMWSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZMWSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZMWSeed_deprecated(uint256& seed);
    bool EraseZMWSeed();
    bool EraseZMWSeed_deprecated();

    bool WriteZMWCount(const uint32_t& nCount);
    bool ReadZMWCount(uint32_t& nCount);
